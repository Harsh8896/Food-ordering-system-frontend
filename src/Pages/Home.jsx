...
// ✅ Image URL fix karne wala helper
const getImageUrl = (image) => {
  if (!image) return "/img/no-image.png"
  if (image.startsWith("http")) return image  // Cloudinary URL
  return `${import.meta.env.VITE_BACKEND_URL}${image}`  // Local fallback
}
          ...
          <div className='d-flex justify-content-center gap-2 mb-4 flex-wrap'>
            {[
              ...
              <button
                ...
                className={`btn btn-sm rounded-pill px-4 ${activeTab === tab.key ? 'btn-warning fw-bold' : 'btn-outline-secondary'}`}
              >
                {tab.label}
              </button>
            ))}
                      ...
                      <div className='p-4 d-flex flex-column'>
                        ...
                        <div className='d-flex justify-content-between align-items-center mt-auto pt-3 border-top'>
                          <span className='fw-bold fs-5'>
                            {item.min_price === 'N/A' ? 'Price N/A' : `₹${item.min_price}`}
                          </span>
